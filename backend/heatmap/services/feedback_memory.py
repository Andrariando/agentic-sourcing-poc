...
Task: Decide a small adjustment to the baseline total that reflects applicable lessons from past reviews ONLY when clearly relevant (same kind of concern: tier, risk, urgency, supplier strategy). 
- score_delta must be a number between -0.5 and 0.5 (inclusive). Use 0 if past notes are not clearly relevant.
- user_note: one concise sentence for a business user (no JSON, no bullet points). If score_delta is 0, say briefly that no strong precedent applied.

Respond with ONLY a JSON object: {{"score_delta": <number>, "user_note": "<string>"}}"""
    ...
    try:
        ...
        delta = float(data.get("score_delta", 0))
